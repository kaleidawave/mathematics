...
theorem mul_must_be_one (a b: Natural) (h1: a ≠ 0) : a = a * b → b = 1 := by
  ...
  | successor b => {
    intro h2
    conv at h2 => rw [multiply.successor]; lhs; rw [←zero.add a];
    rw [add.right_cancel] at h2
...
theorem greater_than_two (n: Natural) : 2 <= n → n ≠ 0 ∧ n ≠ 1 := by
  ...
  rw [h2, ←Natural.add.zero 1, b, add.associative, add.left_cancel _ 1 _, add.commutative, add.one] at h1;
